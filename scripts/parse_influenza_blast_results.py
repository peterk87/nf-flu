...
def parse_blast_result(blast_result: str,
                       df_metadata: pd.DataFrame,
                       regex_subtype_pattern: str,
                       top: int = 3,
                       pident_threshold: float = 90.0) -> Optional[Tuple[pd.DataFrame, Optional[pd.DataFrame], Optional[pd.DataFrame], Dict]]:
...
def find_h_or_n_type(df_merge, seg):
    assert seg in ['4', '6'], 'Can only determine H or N type from segments 4 or 6, respectively!'
...
@click.option('--pident_threshold', default=90.0, help='BLAST percent identity threshold')
...
def report(flu_metadata, blast_results, excel_report, top, pident_threshold, threads):
    ...
    logging.info(f'Parsed Influenza metadata file into DataFrame with n={df_md.shape[0]} rows and n={df_md.shape[1]} columns. There are {unique_subtypes.size} unique subtypes. ')
    regex_subtype_pattern = r'\((H\d+N\d+|' + '|'.join(list(unique_subtypes)) + r')\)'

    pool = Pool(processes=threads)
    logging.info(f'Initialized multiprocessing pool with {threads} processes. Submitting async parsing jobs.')
    async_objects = [pool.apply_async(parse_blast_result, (blast_result, df_md, regex_subtype_pattern), dict(top=top, pident_threshold=pident_threshold)) for blast_result in blast_results]
    ...
    df_all_blast = pd.concat(dfs_blast).rename(columns={k:v for k,v in blast_results_report_columns})
    cols = pd.Series(subtype_results_summary_columns)
    cols = cols[cols.isin(df_subtype_results.columns)]
    df_subtype_predictions = df_subtype_results[cols].rename(columns=subtype_results_summary_final_names)
    ...
    write_excel([('Subtype Predictions', df_subtype_predictions),
...
def write_excel(name_dfs: List[Tuple[str, pd.DataFrame]],
                output_dest: str,
                sheet_name_index: bool = True) -> None:
    ...
    with pd.ExcelWriter(output_dest, engine='xlsxwriter') as writer:
        ...
        for name_df in name_dfs:
            ...
            df.to_excel(writer, sheet_name=fixed_sheetname, index=False, freeze_panes=(1,1))
